import torch
import open_clip
import src_ours.utils as utils
...
class ImageClassifierWithMapping(torch.nn.Module):
    def __init__(self, image_encoder, classification_head, up_proj, linear_mean, linear_var):
        super().__init__()
        self.image_encoder = image_encoder
    ...
    def freeze_head(self):
        ...
        self.linear_mean.weight.requires_grad_(False)
        self.linear_var.weight.requires_grad_(False)

        self.classification_head.weight.requires_grad_(False)
        self.classification_head.bias.requires_grad_(False)

    # def forward(self, inputs):
    #     features = self.image_encoder(inputs).detach()

    #     feature_sub = self.up_proj(features)
    #     feature_sub = self.non_linear_func(feature_sub)
    #     mu = self.linear_mean(feature_sub)
    #     # log_var = self.linear_var(feature_sub)

    #     # std = torch.exp(0.5 * log_var)
    #     # eps = torch.randn_like(std)
    #     # features = eps * std + mu

    #     outputs = self.classification_head(mu)
    #     return outputs
    ...
    def forward(self, inputs):
        ...
        outputs = self.classification_head(feature)
        return outputs
